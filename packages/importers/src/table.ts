...
/** @returns 找不到表头时返回 null（该模板不适用于这个文件） */
...
/**
 * PDF：以"时间列"中的日期文字块为锚点，每个锚点代表一条记录；
 * 各列取与锚点竖直距离在 wrapTolerance 内的文字块，从上到下拼接（处理对手信息折行，P0-1）
 */
function extractFromPdf(items: PdfTextItem[], t: ImportTemplate): Table | null {
  ...
  for (const page of pages) {
    ...
    for (const it of onPage) byY.set(it.y, [...(byY.get(it.y) ?? []), it]);
    const headerLine = [...byY.entries()].find(([, line]) =>
      t.header.required.every((h) => line.some((it) => it.s === h)),
    );
