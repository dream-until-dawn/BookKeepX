...
- 状态：提议中（待确认）；P0 探针已全部验证通过，见 [P0-2~5 结论](../probes/p0-2-5-stack.md)
- 日期：2026-09-24
...
| 图表 | ECharts（**按需引入**） | 中文生态好；✅ P0-5：渲染不是瓶颈，按需引入可减少约 40% 体积 |
...
| 密码 | argon2id（@node-rs/argon2，预编译无需本地编译） | ✅ P0-3：Windows 与 Alpine 容器均使用预编译包，单次哈希约 10 ms |
...
- ✅ P0 探针全部完成：pnpm workspace + Vitest、Drizzle 迁移与 bigint 往返、argon2 安装、表格解析、图表性能。
- 实测补充：postgres.js 的聚合结果为字符串，需经安全整数转换；pnpm 12 需为 esbuild 等显式放行安装脚本。
- 若探针失败，更新本 ADR 再继续。
