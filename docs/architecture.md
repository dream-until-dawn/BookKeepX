...
> 状态：**草案，待确认**。本文描述 MVP（P1）的目标架构，并为 P2/P3 预留扩展点。
...
3. 上传表格导入：微信支付账单、支付宝账单、银行流水、BookKeepX 自定义模板；
...
| `transactions` | id, user_id, direction, **amount_cents(bigint, >0)**, currency, occurred_at(timestamptz), category_id, account_id, counterparty, note, source(manual/import), import_batch_id, external_id, dedupe_key, created_at, updated_at, deleted_at | 核心流水表 |
...
- 去重：`(user_id, source, external_id)` 唯一；无 external_id 时用 `dedupe_key = hash(日期+金额+对方+账户)` 提示疑似重复，由用户决定。

## 5. 导入管线

```
上传文件
  └─► ① 解码：识别编码（UTF-8 / UTF-8-BOM / GBK）→ 统一成字符串或工作表
  └─► ② 识别来源：每个适配器实现 detect(sheet) 打分，取最高分；都不认识 → 走通用"字段映射"
  └─► ③ 解析：适配器把行转成 RawRecord（保留原始列，便于排错）
  └─► ④ 规范化：RawRecord → TransactionDraft（金额转分、时间转 UTC、收支方向判定、过滤"不计收支"/退款等）
  └─► ⑤ 去重 + 分类建议：比对已有流水，套用 category_rules
  └─► ⑥ 预览：返回给前端，用户可改分类、剔除行
...
- 适配器接口：`{ id, detect(input): number, parse(input): RawRecord[], normalize(raw): Result<TransactionDraft, ImportError> }`。

各来源的真实格式（表头位置、编码、金额写法、状态列）**必须由 P0 探针用真实脱敏样本确认**，本文不预设。
